...
from .conv import PartialConv2d
import pdb


_assert_if_size_mismatch = False
...
class UpsampleBlock(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        if (inputs2.size(2) != in1_up.size(2)) or (inputs2.size(3) != in1_up.size(3)):
            ...
            diff2 = (inputs2.size(2) - in1_up.size(2))
            diff3 = (inputs2.size(3) - in1_up.size(3))
            # inputs2_ = inputs2[:, :, diff2 : diff2 + in1_up.size(2), diff3 : diff3 + in1_up.size(3)]
            in1_up = F.pad(in1_up, [diff3 // 2, diff3 - diff3 // 2,
