...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, no_xyz=False, sample_mode="FPRC", sample_rate=0.1, kernal=103):
    ...
    # lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones = [15000, 25000], gamma = 0.8)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # if iteration <= opt.densify_until_iter*2 and iteration > opt.densify_until_iter:
        #     Ll1 = l1_loss(image, gt_image)
        #     ssim_loss = 1.0 - ssim(image, gt_image)
        #         # s3im_loss = s3im(image.flatten(1), gt_image.flatten(1))
        #     image = NN_Comp(image.unsqueeze(0)).squeeze(0)
        #     Ll1_nn = l1_loss(image, gt_image)
        #     ssim_loss_nn = 1.0 - ssim(image, gt_image)
        #     loss = ((1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss + (1.0 - opt.lambda_dssim) * Ll1_nn + opt.lambda_dssim * ssim_loss_nn) / 2.0
        #     loss.backward()
        #     optimizer.step()
        #     lr_scheduler.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[18000, 30000, 40000, 50000, 60000,70000, 80000,90000, 100000, 110000, 120000, 130000, 140000, 150000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[18000, 30000, 40000, 50000, 60000, 80000, 100000, 150000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[40000, 50000, 60000, 80000, 100000, 150000])
